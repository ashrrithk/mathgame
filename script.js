...
const generateQuestion = () => {
    ...
    options[i].setAttribute("style","background-color:#a6c0fe; padding:1rem; border-radius:40px;");
...
for(let i = 0; i<options.length; i++){
    options[i].addEventListener("click", function(){
        for(let j = 0; j<options.length; j++){
            if(j==indexOfAns){
                options[j].setAttribute("style","background-color:green;padding:1rem; border-radius:40px;");
            }
            else{
                options[j].setAttribute("style","background-color:red;padding:1rem; border-radius:40px;");
            }
